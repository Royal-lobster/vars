...
# Unlock (enter PIN once per session)
vars unlock

# Edit values
vars show          # decrypts in-place
# ... edit .vars in your IDE ...
vars hide          # re-encrypts
...
env.DATABASE_URL              // Redacted<string> — safe to log
env.DATABASE_URL.unwrap()     // actual value — explicit opt-in
env.PORT                      // number — already coerced
env.TYPO_VAR                  // TS error — doesn't exist
```

## 🤖 AI Safety

```
cat .vars          → ciphertext (encrypted)
cat .vars.key      → encrypted blob (PIN-protected)
vars show          → requires vars unlock first
vars unlock        → requires human PIN
```
