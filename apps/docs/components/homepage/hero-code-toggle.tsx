		...
		<div className="mt-14 w-full max-w-[620px] min-w-0 text-left">
			...
			<div className="mt-3 flex items-center justify-between px-1 md:hidden">
				<span className="flex items-center gap-2 font-mono text-[11px]">
					<span className="rounded border border-green-500/20 bg-green-500/10 px-2 py-0.5 text-green-400">
						vars show
					</span>
			...
			<div className="hidden md:block">
				...
				>
					...
					<div className="relative">{vault}</div>

					{/* Unlocked (plaintext) layer — clipped */}
					<div className="absolute inset-0 overflow-hidden" style={{ width: `${position}%` }}>
						<div style={{ width: containerRef.current?.offsetWidth ?? "100%" }}>{unlocked}</div>
					</div>
