...
const codeBlockStyle =
	"[&_figure]:!my-0 [&_figure]:!rounded-lg [&_pre]:!text-[12px] [&_pre]:!leading-[1.8]";

function SectionBg({ image, side }: { image: string; side: "left" | "right" }) {
	return (
		<div
			className={`absolute inset-0 overflow-hidden pointer-events-none ${side === "left" ? "-left-20" : "-right-20"}`}
		>
			<img
				src={image}
				alt=""
		...
		<section className="px-5 py-24 md:px-10">
			<div className="mb-16 text-center">
				<h2 className="text-[clamp(28px,4vw,38px)] font-bold tracking-[-1.5px]">
					What no other tool <em className="font-serif italic text-green-500 font-normal">does.</em>
				</h2>
			</div>
					...
					<div className="relative z-10 mx-auto grid max-w-[1120px] items-center gap-10 md:grid-cols-2">
						<div>
							...
							<p className="mt-4 text-[15px] leading-relaxed text-white/50">
								t3-env gives you schema validation. Doppler gives you a secrets vault. vars gives
								you both — in a single file you commit to git. The schema{" "}
								<em className="text-white/70">is</em> the documentation. New devs read it and know
								exactly what every variable needs, what type it is, and which environments it spans.
							</p>
						</div>
					...
					<div className="relative z-10 mx-auto grid max-w-[1120px] items-center gap-10 md:grid-cols-2">
						...
						<div className="order-1 md:order-2">
							...
							<p className="mt-4 text-[15px] leading-relaxed text-white/50">
								...
								variable. That&apos;s it. No more pasting 12 secrets into Vercel one by one. No more
								hoping staging matches prod. Add a secret? Edit, commit, push — CI picks it up
								automatically.
							</p>
						</div>
					</div>
				...
				<div className="relative overflow-hidden border-t border-b border-white/[0.04] py-20">
					...
					<div className="relative z-10 mx-auto grid max-w-[1120px] items-center gap-10 md:grid-cols-2">
						<div>
							...
							<p className="mt-4 text-[15px] leading-relaxed text-white/50">
								When an AI coding agent tries to decrypt your .vars file, it hits a native system
								dialog — a real OS-level prompt that requires your PIN. No session caching, no token
								persistence. Every single decryption needs your explicit approval. The agent sees
