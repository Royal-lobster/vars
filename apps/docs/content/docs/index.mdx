...
**Type safety.** Each variable has a schema — `z.string().url()`, `z.number().min(1024)`, `z.enum(["debug", "info", "warn"])`. These are [Zod](https://zod.dev) expressions, a popular TypeScript validation library. If someone puts a typo in the database URL, vars catches it at build time, not after it crashes in prod.
...
- You're a **small to mid-size team** (2–15 people) sharing secrets across environments
- You want **type-safe config** without a SaaS dependency
- You're tired of **.env file sprawl** and Slack-based secret sharing
