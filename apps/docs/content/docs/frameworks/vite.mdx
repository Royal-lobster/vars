---
title: Vite / React
description: Using vars with Vite for server-side SSR and client-side bundles.
icon: SiVite
---
...
`process.env` works in SSR code. Import from `#vars` for type safety.

## Client-Side

Vite uses `import.meta.env` in the browser, not `process.env`. Two options:
