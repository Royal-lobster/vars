---
title: File Format
description: The .vars file format — from simple key-value pairs to full config systems, one level at a time.
icon: FileCode
---
...
No flags needed — `vars run` picks it up automatically and shows a notice:
...
- **Full syntax** — env blocks, groups, schemas, interpolation, `use` imports all work.
- **New variables allowed** — local can add variables that don't exist in the base file.
- **`env()` and `param` inherited from base** — if your local file declares them, they're ignored with a warning. The team's environment and parameter structure stays authoritative.
- **Top-level only** — only the entry-point file gets a local overlay. Files reached via `use` imports don't automatically pick up their own `.local.vars`.
- **Never encrypted** — local files are plaintext and gitignored. No PIN needed to edit them.
- **Naming convention** — follows the `<name>.<modifier>.vars` pattern: `config.vars` → `config.local.vars`, same as `config.unlocked.vars`.

`vars init` adds `*.local.vars` to your `.gitignore` automatically. The pre-commit hook blocks accidental commits of local files.
...
| `config.local.vars` | Per-developer override file (gitignored, never encrypted) |
