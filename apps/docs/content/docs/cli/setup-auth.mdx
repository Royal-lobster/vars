...
Installs a pre-commit hook that blocks commits when any `config.vars` file has `@vars-state unlocked` in its header. This prevents accidentally committing decrypted secrets.
