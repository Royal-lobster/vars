...
```bash
npx dotvars run --env <env> [--param key=val] -- <command> [args...]
```
...
npx dotvars run --env prod --param DATABASE_URL=postgres://localhost/mydb -- node scripts/migrate.js
```
...
npx dotvars gen config.vars --output src/lib/env.ts
```
