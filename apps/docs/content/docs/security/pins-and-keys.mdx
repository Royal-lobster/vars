...
For trusted automation, PIN-taking commands also accept `--pin <pin>` and the `VARS_PIN` environment variable. An explicit `--pin` takes precedence over `VARS_PIN`. This deliberately gives the process—and any agent controlling it—access to the encrypted values.

<Callout type="warn">A PIN passed with `--pin` is visible in process arguments and may be recorded in shell history, agent transcripts, and command logs. Use it only when you trust the automation with every secret that PIN can decrypt. Prefer an owner-scoped PIN when the automation needs only part of the file.</Callout>
