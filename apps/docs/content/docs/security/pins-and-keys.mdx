...
`--pin` takes precedence over `--pin-file`, followed by `VARS_PIN` and `VARS_PIN_FILE`. `--key-file` takes precedence over `VARS_KEY_FILE`, then vars finds the nearest `.varskey`. Selecting any PIN or key-file source makes the encrypted envelope take precedence over the raw `VARS_KEY` compatibility fallback.
