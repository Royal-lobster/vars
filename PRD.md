...
`@vars/core` depends on `zod` and `argon2`. Framework packages depend on `@vars/core` + their framework as a peer dep. No shared plugin interface — each package directly imports what it needs from core.

### 14.4 Key Dependencies

**@vars/core:**
```json
{
  "dependencies": { "zod": "^3.x", "argon2": "^0.x" },
  "devDependencies": { "tsup": "^8.x", "vitest": "^3.x", "typescript": "^5.x" }
}
```
...
```json
{
  "dependencies": { "@vars/core": "workspace:*", "citty": "^0.2.x" }
}
```
...
```json
{
  "dependencies": { "@vars/core": "workspace:*" },
...
All packages output ESM (`dist/index.mjs`), CJS (`dist/index.cjs`), and types (`dist/index.d.ts`) via tsup.
