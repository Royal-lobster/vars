import type { CheckExpr, CheckPredicate } from "./types.js";
...
function evalPredicate(
	...
	switch (pred.kind) {
		...
		case "logical":
			...
			return (
				evalPredicate(pred.left, vars, env, params) || evalPredicate(pred.right, vars, env, params)
			);
		case "not":
...
function compare(
	...
	const useNum = !Number.isNaN(numLeft) && !Number.isNaN(numRight) && op !== "==" && op !== "!=";
...
function evalFunctionCall(
	...
	switch (name) {
		case "defined": {
			const varName =
				args[0]!.kind === "var_ref" ? (args[0] as { kind: "var_ref"; name: string }).name : "";
