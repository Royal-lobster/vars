...
class Tokenizer {
	...
	private addToken(type: TokenType, value: string, tokenLine: number, tokenCol: number): void {
	...
	private trySchemaCapture(): void {
		...
		if (this.src[lookahead] !== "z" || this.src[lookahead + 1] !== ".") {
			return; // not a schema
		}
	...
	private captureTripleString(startLine: number, startCol: number): void {
		...
		if (end === -1) {
			...
			this.addToken(TokenType.ERROR, `"""${this.src.slice(contentStart)}`, startLine, startCol);
			this.pos = this.src.length;
			return;
		}
		...
		this.addToken(TokenType.TRIPLE_STRING, stripped.join("\n"), startLine, startCol);
	}
	...
	tokenize(): Token[] {
		while (this.pos < this.src.length) {
			...
			if (ch === "{") {
				this.pos++;
				this.addToken(TokenType.LBRACE, "{", tokenLine, tokenCol);
				continue;
			}
			if (ch === "}") {
				this.pos++;
				this.addToken(TokenType.RBRACE, "}", tokenLine, tokenCol);
				continue;
			}
			if (ch === "(") {
				this.pos++;
				this.addToken(TokenType.LPAREN, "(", tokenLine, tokenCol);
				continue;
			}
			if (ch === ")") {
				this.pos++;
				this.addToken(TokenType.RPAREN, ")", tokenLine, tokenCol);
				continue;
			}
			if (ch === "[") {
				this.pos++;
				this.addToken(TokenType.LBRACKET, "[", tokenLine, tokenCol);
				continue;
			}
			if (ch === "]") {
				this.pos++;
				this.addToken(TokenType.RBRACKET, "]", tokenLine, tokenCol);
				continue;
			}
			if (ch === ",") {
				this.pos++;
				this.addToken(TokenType.COMMA, ",", tokenLine, tokenCol);
				continue;
			}
			if (ch === ".") {
				this.pos++;
				this.addToken(TokenType.DOT, ".", tokenLine, tokenCol);
				continue;
			}
			...
			if (ch >= "0" && ch <= "9") {
				...
				while (
					this.pos < this.src.length &&
					((this.src[this.pos]! >= "0" && this.src[this.pos]! <= "9") || this.src[this.pos] === ".")
				) {
					num += this.advance();
				}
