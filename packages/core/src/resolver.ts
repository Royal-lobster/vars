...
export function resolveInterpolation(template: string, resolved: Map<string, string>): string {
	...
	for (const decl of declarations) {
		if (decl.kind === "variable") {
			...
			for (const varDecl of decl.declarations) {
				...
				const flatName = alreadyPrefixed ? varDecl.name : `${groupPrefix}${varDecl.name}`;
