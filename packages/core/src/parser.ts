	...
	while (!cursor.isEOF() && !cursor.check(TokenType.RBRACE)) {
		...
		if (t.type === TokenType.KEYWORD && t.value === "when") {
			...
			if (envs.length > 0 && !envs.includes(envName)) {
				errors.push(new ParseError(`Undeclared environment '${envName}'`, entryLine, filePath));
			}
...
function parseValue(cursor: TokenCursor): Value {
	...
	if (t.type === TokenType.NUMBER) {
		cursor.advance();
		const num = t.value.includes(".") ? Number.parseFloat(t.value) : Number.parseInt(t.value, 10);
...
function parseArrayValue(cursor: TokenCursor): LiteralValue {
	...
	while (!cursor.isEOF() && !cursor.check(TokenType.RBRACKET)) {
		...
		if (t.type === TokenType.STRING) {
			...
			cursor.advance();
			items.push(t.value.includes(".") ? Number.parseFloat(t.value) : Number.parseInt(t.value, 10));
...
function parseCheckExpr(cursor: TokenCursor): CheckExpr {
	...
	if (t.type === TokenType.NUMBER) {
		...
		return {
			kind: "number_literal",
			value: t.value.includes(".") ? Number.parseFloat(t.value) : Number.parseInt(t.value, 10),
		};
	}
	...
	throw cursor.error(`Expected check expression, got ${TokenType[t.type]} '${t.value}'`);
}
