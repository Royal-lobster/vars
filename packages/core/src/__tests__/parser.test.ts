...
describe("parser", () => {
	...
	describe("variable declarations", () => {
		...
		it("parses variable with explicit schema and default", () => {
			const result = parse(fixture("simple.vars"));
			const port = result.ast.declarations.find((d) => d.kind === "variable" && d.name === "PORT");
		...
		it("parses variable with default AND env overrides", () => {
			...
			if (logLevel?.kind === "variable") {
				...
				if (logLevel.value?.kind === "env_block") {
					const defaultEntry = logLevel.value.entries.find((e) => e.env === "*");
	...
	describe("checks", () => {
		it("parses check blocks with descriptions", () => {
			...
			expect(result.ast.checks[0]!.description).toBe("No debug logging in prod");
			expect(result.ast.checks[1]!.description).toBe("Debug flag consistency");
		});
	...
	describe("arrays", () => {
		it("parses array values in env blocks", () => {
			...
			expect(result.errors).toHaveLength(0);
			const cors = result.ast.declarations.find((d) => d.kind === "variable" && d.name === "CORS");
