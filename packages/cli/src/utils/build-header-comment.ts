...
export function buildHeaderComment(ctx: HeaderCommentContext): string {
	...
	if (ctx.source === "boilerplate") {
		...
		if (ctx.detectedPrefixes.length > 0) {
			lines.push(
				`# Variables with ${ctx.detectedPrefixes.join(", ")} prefixes were marked public.`,
			);
			lines.push("#");
			...
			lines.push(
				"# If any of these should be secret, remove the `public` keyword to enable encryption.",
			);
		}
	}
