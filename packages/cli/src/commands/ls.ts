		...
		if (args.file) {
			...
			for (const v of resolved.vars) {
				...
				if (v.metadata?.expires) {
					...
					meta += expired
						? pc.red(` expired:${v.metadata.expires}`)
						: pc.dim(` expires:${v.metadata.expires}`);
				}
			...
			listFilesInDirectory(getProjectRoot());
		}
	},
});

function listFilesInDirectory(root: string): void {
	...
	for (const f of files) {
		...
		for (const d of result.ast.declarations) {
			...
			if (d.kind === "variable" && d.metadata?.expires && new Date(d.metadata.expires) < new Date())
				warns++;
		}
