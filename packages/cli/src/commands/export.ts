...
export default defineCommand({
	...
	args: {
		...
		format: {
			type: "string",
			default: "dotenv",
			description: "Output format: dotenv, json, k8s-secret",
		},
	...
	async run({ args }) {
		...
		if (resolved.envs.length > 0 && !resolved.envs.includes(env)) {
			console.error(
				pc.red(
					`  Unknown environment "${env}". Declared environments: ${resolved.envs.join(", ")}`,
				),
			);
			process.exit(1);
		}
		...
		switch (args.format) {
			...
			case "json": {
				...
				for (const [k, v] of pairs) {
					...
					if (
						varDef &&
						(varDef.schema.includes("z.number") || varDef.schema.includes("z.coerce.number"))
					) {
						obj[k] = Number(v);
					} else if (
						varDef &&
						(varDef.schema.includes("z.boolean") || varDef.schema.includes("z.coerce.boolean"))
					) {
						obj[k] = v === "true";
					} else {
						obj[k] = v;
