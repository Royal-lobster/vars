...
describe("@vars/turbo", () => {
  ...
  describe("discoverWorkspaceVarsFiles", () => {
    it("finds .vars files in root and workspace packages", () => {
      ...
      mockReaddirSync.mockImplementation((path) => {
        const p = String(path);
        if (p.endsWith("/apps")) return [{ name: "web", isDirectory: () => true }] as unknown as ReturnType<typeof readdirSync>;
        if (p.endsWith("/packages")) return [{ name: "core", isDirectory: () => true }] as unknown as ReturnType<typeof readdirSync>;
    ...
    it("returns results for each discovered .vars file", () => {
      ...
      mockReaddirSync.mockImplementation((path) => {
        if (String(path).endsWith("/apps")) {
          return [{ name: "web", isDirectory: () => true }] as unknown as ReturnType<typeof readdirSync>;
        }
        return [] as unknown as ReturnType<typeof readdirSync>;
      });
  ...
  describe("diffApps", () => {
    it("returns variables present in one app but not the other", () => {
      ...
      mockReadFileSync
        .mockReturnValueOnce("file-a-content")
        .mockReturnValueOnce("file-b-content");
      mockParse
    ...
    it("returns empty arrays when both apps have same variables", () => {
      const parsed = {
        variables: [
          { name: "PORT", schema: "z.number()", values: [], metadata: {}, line: 1 },
        ],
        refines: [],
        extendsPath: null,
      };
