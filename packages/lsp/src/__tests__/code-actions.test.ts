...
import { CodeActionContext, computeCodeActions } from "../code-actions.js";
...
describe("code-actions", () => {
	...
	describe("mark as deprecated", () => {
		it("suggests adding @deprecated when variable has no deprecation", () => {
			const text = ["OLD_TOKEN  z.string()", "  @dev = abc123"].join("\n");
			const actions = computeCodeActions({
				text,
				startLine: 0,
				endLine: 1,
			...
			const deprecateActions = actions.filter((a) => a.kind === "refactor.deprecated");
