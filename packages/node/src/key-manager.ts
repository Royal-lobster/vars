...
import { ALGORITHM, IV_LENGTH, TAG_LENGTH, KEY_LENGTH } from "@vars/core";
...
export async function encryptMasterKey(masterKey: Buffer, pin: string): Promise<string> {
  ...
  const cipher = createCipheriv(ALGORITHM, Buffer.from(wrappingKey), iv, { authTagLength: TAG_LENGTH });
...
export async function decryptMasterKey(encoded: string, pin: string): Promise<Buffer> {
  ...
  const decipher = createDecipheriv(ALGORITHM, Buffer.from(wrappingKey), iv, { authTagLength: TAG_LENGTH });
